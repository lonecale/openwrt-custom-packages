...
# rm -rf webcn
rm -rf luci-app-netdata/{.git,po/zh_Hant}
...
#sed -i '/#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 12, 0))/,/#endif/ s/KERNEL_VERSION(6, 12, 0)/KERNEL_VERSION(6, 11, 0)/' luci-app-oaf/oaf/src/af_log.c
sed -i '/#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 12, 0))/,/#endif/c\    { },' luci-app-oaf/oaf/src/af_log.c
rm -rf luci-app-oaf/.git
...
git clone https://github.com/kiddin9/op-packages kwrt-packages -b main --depth 1
...
# mv immortalwrt-packages/devel/rust-bindgen ./smartdns
sed -i 's|include ../../lang/rust/rust-package.mk|include $(TOPDIR)/feeds/packages/lang/rust/rust-package.mk|' smartdns/Makefile
# sed -i 's|include ../../lang/rust/rust-host-build.mk|include $(TOPDIR)/feeds/packages/lang/rust/rust-host-build.mk|' smartdns/rust-bindgen/Makefile
